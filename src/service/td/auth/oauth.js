...
const oauthService = {
  getRefreshToken: async (encodedAuthCode) => {
    return authServiceDelegate
      .postOauth2Token(mapToAuthorizationCodeGrantRequest(encodedAuthCode))
      .then((response) => {
  ...
  getAccessToken: async (refreshToken) => {
    return authServiceDelegate
      .postOauth2Token(mapToRefreshTokenGrantRequest(refreshToken))
      .then((response) => {
        return response.data;
      })
      .catch(() => {
